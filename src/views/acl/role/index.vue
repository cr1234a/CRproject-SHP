  ...
  <div>
    ...
    <el-table border stripe style="width: 960px" v-loading="listLoading" :data="roles"
      ...
      <el-table-column label="操作" width="300" align="center">
        <template slot-scope="{row}">
          <BintButton size="mini" type="info" icon="el-icon-info" title="分配权限" @click='move(row)'
            style='margin-right:10px' />
          <BintButton size="mini" type="primary" icon="el-icon-check" title="确定" @click="updateRole(row)"
            v-if="row.edit" style='margin-right:10px' />
          <BintButton size="mini" type="primary" icon="el-icon-edit" title="修改角色" @click="row.edit = true"
            v-if="!row.edit" style='margin-right:10px' />
          <BintButton size="mini" type="danger" icon="el-icon-delete" title="删除角色" @click="removeRole(row)"
            style='margin-right:10px' />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    */
    updateRole(role) {
      this.$api.role.saveOrUpdateRole({ id: role.id, roleName: role.roleName })
        .then(result => {
    ...
    */
    addRole() {
      ...
      this.$prompt('请输入新名称', '添加角色', {
        ...
        this.$api.role.saveOrUpdateRole({ roleName: value }).then(result => {
