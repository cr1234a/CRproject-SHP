    ...
    <el-table border stripe v-loading="listLoading" :data="users" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="{row}">
          <BintButton type="info" size="mini" icon="el-icon-user-solid" title="分配角色" @click="showAssignRole(row)"
            style='margin-right:10px' />
          <BintButton type="primary" size="mini" icon="el-icon-edit" title="修改用户" @click="showUpdateUser(row)" />
          <el-popconfirm :title="`确定删除 ${row.username} 吗?`" @onConfirm="removeUser(row.id)">
            <BintButton style="margin-left:10px" slot="reference" type="danger" size="mini" icon="el-icon-delete"
              title="删除用户" />
          </el-popconfirm>
        </template>
      </el-table-column>
    ...
    */
    revomveUsers() {
      this.$confirm('确定删除吗?').then(async () => {
        console.log(1)
        const res = await this.$api.user.removeUsers(this.selectedIds)
        console.log(2)
        console.log(res)
    ...
    addOrUpdate() {
      this.$refs.userForm.validate(valid => {
        if (valid) {
          const { user } = this
          this.loading = true
          this.$api.user.addOrUpdateUser(user).then((result) => {
            this.loading = false
