<template>
  <div>
    ...
    <el-table border style="margin-bottom: 20px;" :data="menuPermissionList" :expand-row-keys="expandKeys" row-key="id">
      ...
      <el-table-column prop="toCode" label="跳转权限值" align="center" />

      <el-table-column label="操作" align="center">
        <template slot-scope="{row}">

          <BintButton :disabled="row.level === 4" type="primary" icon="el-icon-plus" size="mini"
            @click="toAddPermission(row)" :title="getAddTitle(row.level)" style='margin-right:10px' />

          <BintButton type="primary" icon="el-icon-edit" size="mini" :disabled="row.level === 1"
            @click="toUpdatePermission(row)" :title="row.level === 4 ? '修改功能' : '修改菜单'" style='margin-right:10px' />

          <BintButton :disabled="row.level === 1" type="danger" icon="el-icon-delete" size="mini"
  ...
  methods: {
    ...
    */
    async fetchPermissionList() {
      const result = await this.$api.permission.getPermission()
    ...
    */
    addOrUpdatePermission() {
      this.$refs.permission.validate(async valid => {
        if (valid) {
          // pname不需要携带[this.permission.perm.id ? 'updatePermission' : 'addPermission']
          const result = await this.$api.permission.addOrUpdatePermission(this.permission)
          this.$message.success(result.message || `${this.permission.id ? '修改' : '添加'}成功!`)
          this.resetData()
          this.fetchPermissionList()
        }
      })
